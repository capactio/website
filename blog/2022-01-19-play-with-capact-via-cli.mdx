...
Our first blog post focused strongly on **_why_**. Now it's time for **_how_**. There are many ways to [get started with Capact](/docs/next/getting-started). Today we will focus on your perspective as a
Capact User.
...
Action is the entry point for executing any Interface. It allows you to define which Interface should be executed and with which parameters. This is later consumed by Capact Engine and the appropriate workflow is rendered. You can review it and approve it for execution.
...
2. List available Interfaces:
  ...
  We are interested in this part:
    ...
    ---------------------------------------------+-----------------+-----------------------------------------------
     cap.interface.capactio.examples.say            0.1.0          cap.implementation.capactio.examples.ricky
                                                                      cap.implementation.capactio.examples.hello
    ```
...
Execute the Action with the Policy:
...
To test it, execute [steps](#steps) once again, but now select the **Policy-Adjusted Execution** tab.
