...
Capact consists of multiple components, which you can find in [`deploy/kubernetes/charts`](https://github.com/capactio/capact/tree/main/deploy/kubernetes/charts).

One of the components is Neo4j. Unfortunately, its usage, significantly increases CPU and memory consumption. Due to this fact, you need to make sure that Docker has enough resource to install Capact properly.
...
Once you have your local Kubernetes cluster set up, install Capact. You can choose from two types of release channels: **Stable** and **Latest**:

- The **Stable** channel refers to the official [Capact releases](https://github.com/capactio/capact/releases).
- The **Latest** channel refers to the latest successful build from the commit on the [Capact repository `main` branch](https://github.com/capactio/capact/commits/main).
...
:::note
This command deletes the local cluster with all data, including history of executed Actions and created TypeInstances. Make sure you cleaned up all external resources (e.g. managed PostgreSQL databases) based on the TypeInstance data.
:::
