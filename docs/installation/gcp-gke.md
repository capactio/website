...
1. Check the latest stable Capact release on [Capact GitHub releases](https://github.com/Project-Voltron/go-voltron/releases) page.
   
   Note the git tag for the latest version (for example, `v0.3.0`).

1. Clone the `go-voltron` repository for a specific tag noted from previous version.
	
	```bash
	git clone --depth 1 --branch {tag} https://github.com/Project-Voltron/go-voltron.git
	cd ./go-voltron
	```
...
By default, the OCH manifests are synchronized with the `och-content` directory from the `go-voltron` repository on a specific release branch. You can change that by overriding **MANIFEST_PATH** environment variable for **och-public** Deployment.
     
For example, to use the `master` branch as a source of OCH manifests, run:
