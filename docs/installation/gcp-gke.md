...
### Change the sources of Hub manifests

By default, the Hub manifests are synchronized with the `manifests` directory from the `capact` repository on a specific release branch. You can change that by overriding **MANIFESTS_SOURCES** environment variable for **hub-public** Deployment.
...
kubectl set env deployment/capact-hub-public -n capact-system --containers="hub-public-populator" MANIFESTS_SOURCES="git@github.com:capactio/hub-manifests.git?sshkey=${SSH_KEY}&ref=${BRANCH_NAME}"
```
...
> **NOTE:** To use more manifest sources, separate them using comma.