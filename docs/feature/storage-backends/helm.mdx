...
The Helm backend storage gives you an option to map the TypeInstance's value to a given Helm release. This solves problems with outdated static data about Helm release that are stored in built-in database. The functionality in a form of a dedicated storage backend is installed using Capact Action. This guide describes how to install and configure the Helm storage backends for Capact Local Hub.
