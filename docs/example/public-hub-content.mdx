...
3. Execute `populator`:
   ...
   >**NOTE**: More information about using `populator`, can be found in [`populator_register-ocf-manifests.md`](https://github.com/capactio/capact/blob/main/cmd/populator/docs/populator_register-ocf-manifests.md).

## Using many manifests sources
...
export SOURCE_REPO_2_URL="{url}" # e.g. github.com/capactio/hub-manifests
# The git ref to checkout. It can point to a commit SHA, a branch name, or a tag.
export SOURCE_REPO_2_REF="{ref}" # e.g. main

capact install --capact-overrides="hub-public.populator.enabled=true,hub-public.populator.manifestsLocations[0].repository=${SOURCE_REPO_1_URL},hub-public.populator.manifestsLocations[0].branch=${SOURCE_REPO_1_REF},hub-public.populator.manifestsLocations[1].repository=${SOURCE_REPO_2_URL},hub-public.populator.manifestsLocations[0].branch=${SOURCE_REPO_2_REF}"
```

>**NOTE**: Merging of the manifests is based on manifest paths. Thus, they should be unique in many locations.
