...
```shell
curl -Lo capact.exe https://github.com/capactio/capact/releases/download/v0.7.0/capact-windows-amd64.exe
```
...
ghcr.io/capactio/tools/capact-cli:v0.7.0 login $GATEWAY_URL -u $USERNAME -p $PASSWORD
```
...
```shell
curl -Lo capact https://github.com/capactio/capact/releases/download/v0.7.0/capact-$OS-$ARCH
```
